...
struct RenderObject {
    ...
    int shaderType;     // 0: Normal, 1: Stripes
    bool isTransparent;
    float depth;        // sort
};
...
ID3D12PipelineState* g_psoArray[2][2]; // [ShaderIdx][BlendIdx] 0:Opaque, 1:Alpha
...
// cameras
XMFLOAT3 g_camPos = { 0.0f, 2.0f, -10.0f };
float g_camYaw = 0.0f, g_camPitch = 0.0f;
std::vector<RenderObject> g_objects;

const char* g_shaderSource = R"(
struct SceneCB { float4x4 viewProj; };
struct ObjectCB { 
    float4x4 world; // 64 byte (OK)
    float4 color;   // 16 byte (OK)
    uint type;      // 4 byte
    float3 padding; // up to 16 byte
};

ConstantBuffer<SceneCB> scene : register(b0);
ConstantBuffer<ObjectCB> obj : register(b1);
...
VOut VSMain(float3 pos : POSITION, float2 uv : TEXCOORD) {
    VOut vout;
    float4 wpos = mul(float4(pos, 1.0f), obj.world);
...
float4 PSMain(VOut pin) : SV_Target {
    if (obj.type == 1 && distance(pin.uv, float2(0.5, 0.5)) > 0.5) discard;
    return obj.color;
}

float4 PSStripes(VOut pin) : SV_Target {
    if (obj.type == 1 && distance(pin.uv, float2(0.5, 0.5)) > 0.5) discard;
    float stripes = frac(pin.uv.y * 10.0) > 0.5 ? 1.0 : 0.8;
    return obj.color * stripes;
}
)";
...
void InitD3D() {
    ...
    D3D12_HEAP_PROPERTIES depthHeapProp = { D3D12_HEAP_TYPE_DEFAULT };
    D3D12_RESOURCE_DESC depthDesc = { D3D12_RESOURCE_DIMENSION_TEXTURE2D, 0, 1280, 720, 1, 1, DXGI_FORMAT_D32_FLOAT, {1, 0}, D3D12_TEXTURE_LAYOUT_UNKNOWN, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL };
    ...
    rootParameters[0].Constants = { 0, 0, 16 }; // Scene Matrix
    rootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;
    rootParameters[1].Constants = { 1, 0, 21 }; // Object Data (16+4+1)

    D3D12_ROOT_SIGNATURE_DESC rootSigDesc = { 2, rootParameters, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT };
    ID3DBlob* signature;
    ...
    ID3DBlob* vs, * psNormal, * psStripes, * errorBlob = nullptr;
    HRESULT hr = D3DCompile(g_shaderSource, strlen(g_shaderSource), nullptr, nullptr, nullptr, "VSMain", "vs_5_1", 0, 0, &vs, &errorBlob);
    if (FAILED(hr)) {
        if (errorBlob) {
    ...
    D3DCompile(g_shaderSource, strlen(g_shaderSource), nullptr, nullptr, nullptr, "PSMain", "ps_5_1", 0, 0, &psNormal, nullptr);
    D3DCompile(g_shaderSource, strlen(g_shaderSource), nullptr, nullptr, nullptr, "PSStripes", "ps_5_1", 0, 0, &psStripes, nullptr);
    ...
    // create PSO
    ID3DBlob* psList[] = { psNormal, psStripes };
    for (int s = 0; s < 2; s++) {
        psoDesc.PS = CD3DX12_SHADER_BYTECODE(psList[s]);
        // opaque
    ...
    D3D12_RESOURCE_DESC vtxDesc = { D3D12_RESOURCE_DIMENSION_BUFFER, 0, sizeof(verts), 1, 1, 1, DXGI_FORMAT_UNKNOWN, {1,0}, D3D12_TEXTURE_LAYOUT_ROW_MAJOR, D3D12_RESOURCE_FLAG_NONE };
    g_device->CreateCommittedResource(&uploadHeap, D3D12_HEAP_FLAG_NONE, &vtxDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&g_vertexBuffer));
    void* pData;
    ...
    for (int i = 0; i < OBJECT_COUNT; i++) {
        RenderObject obj;
        obj.position = { (float)(rand() % 20 - 10), (float)(rand() % 10 - 5), (float)(rand() % 20) };
        obj.color = { (rand() % 100) / 100.f, (rand() % 100) / 100.f, (rand() % 100) / 100.f, (rand() % 10 > 6) ? 0.5f : 1.0f };
...
void Update() {
    ...
    if (GetAsyncKeyState('W')) XMStoreFloat3(&g_camPos, XMLoadFloat3(&g_camPos) + forward * speed);
    if (GetAsyncKeyState('S')) XMStoreFloat3(&g_camPos, XMLoadFloat3(&g_camPos) - forward * speed);
    if (GetAsyncKeyState('D')) XMStoreFloat3(&g_camPos, XMLoadFloat3(&g_camPos) + right * speed);
    if (GetAsyncKeyState('A')) XMStoreFloat3(&g_camPos, XMLoadFloat3(&g_camPos) - right * speed);

    XMMATRIX view = XMMatrixLookAtLH(XMLoadFloat3(&g_camPos), XMLoadFloat3(&g_camPos) + forward, XMVectorSet(0, 1, 0, 0));
    XMMATRIX proj = XMMatrixPerspectiveFovLH(XM_PIDIV4, 1280.f / 720.f, 0.1f, 100.f);
    ...
    std::sort(g_objects.begin(), g_objects.end(), [](const RenderObject& a, const RenderObject& b) {
    ...
    D3D12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(g_renderTargets[g_frameIndex], D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
    ...
    for (auto& obj : g_objects) {
        ...
        g_commandList->SetGraphicsRoot32BitConstants(1, 21, &cb, 0);
        g_commandList->DrawInstanced(6, 1, 0, 0);
    }

    barrier = CD3DX12_RESOURCE_BARRIER::Transition(g_renderTargets[g_frameIndex], D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
    g_commandList->ResourceBarrier(1, &barrier);
    g_commandList->Close();
...
int main(int argc, char** argv)
{
    ...
    WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_HREDRAW | CS_VREDRAW, WndProc, 0, 0, hInst, nullptr, nullptr, nullptr, nullptr, L"D3D12Sample", nullptr };
    RegisterClassEx(&wc);
    g_hWnd = CreateWindow(wc.lpszClassName, L"D3D12 Multi-Object Sample (WASD to Move)", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 1280, 720, nullptr, nullptr, hInst, nullptr);
